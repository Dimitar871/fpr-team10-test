    ...
    <main class="w-full h-full flex flex-col gap-4 max-w-[800px] mx-auto py-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex flex-col gap-1 items-start">
                <p class="text-xl font-semibold">Create New Article</p>
                <span class="text-[var(--text-color)] text-xs">Add details for your new article</span>
            </div>
        </div>
        ...
        <div id="messageBox" class="hidden fixed top-4 right-4 px-4 py-2 rounded shadow-md z-50"></div>

        <!-- Article Form -->
        <form method="POST" action="{{ route('articles.store') }}" class="bg-[var(--main-color)] border-[var(--background-color)] p-6 rounded-md shadow-sm mt-4">
            @csrf

            <!-- Labels Field -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-[var(--text-color)] mb-1">Labels*</label>
                <div class="flex flex-wrap gap-2 max-h-40 overflow-auto">
                        ...
                        <div class="flex items-center gap-2 mb-2" data-label-id="{{ $label->id }}">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="labels[]" value="{{ $label->id }}"
                                       class="form-checkbox"
                                    {{ (is_array(old('labels')) && in_array($label->id, old('labels'))) ? 'checked' : '' }}>
                                <span class="ml-2 text-[var(--text-color)] label-name">{{ $label->name }}</span>
                                <button type="button" class="ml-1 text-[var(--text-color)] edit-label-btn" title="Edit">&#9998;</button>
                                <button type="button"
                            ...
                            <input type="text" class="hidden edit-label-input w-full px-3 py-2 border border-[var(--text-color)] rounded-md"
                                   value="{{ $label->name }}">
                    ...
                    <label class="inline-flex items-center mr-4">
                        <input type="checkbox" id="otherCheckbox" class="form-checkbox" {{ old('other_label') ? 'checked' : '' }}>
                        <span class="ml-2 text-[var(--text-color)]">Add custom label</span>
                    </label>
                </div>
                @error('labels')
                ...
                <input type="text" id="other_label" name="other_label"
                           ...
                           class="w-full px-3 py-2 border @error('other_label') border-[var(--delete-color)] @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--sub-color)] focus:border-transparent"
            ...
            <div class="mb-6">
                ...
                <input type="text" id="title" name="title"
                       value="{{ old('title') }}"
                       class="w-full px-3 py-2 border @error('title') text-[var(--delete-color)] @else border-[var(--text-color)] @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--sub-color)] focus:border-transparent"
            ...
            <div class="mb-6">
                ...
                <textarea id="excerpt" name="excerpt" rows="2"
                          class="w-full px-3 py-2 border @error('excerpt') border-red-500 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-[#3aa499] focus:border-transparent"
                          placeholder="Give a short excerpt what your article is about">{{ old('excerpt') }}</textarea>
                @error('excerpt')
            ...
            <div class="mb-6">
                ...
                <textarea id="content" name="content" rows="5"
                          class="w-full px-3 py-2 border @error('content') border-red-500 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-[#3aa499] focus:border-transparent"
            ...
            <div class="mb-6">
                ...
                <input id="author" name="author"
                       value="{{ old('author') }}"
                       class="w-full px-3 py-2 border @error('author') border-red-500 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-[#3aa499] focus:border-transparent"
